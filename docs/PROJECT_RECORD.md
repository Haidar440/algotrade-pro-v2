...
4. [Frontend Files — React (Pre-existing)](#frontend-files--react-pre-existing)
5. [Documentation & Config Files](#documentation--config-files)
6. [Legacy Files — Pending Cleanup](#legacy-files--pending-cleanup)
...
│   │   │   └── schemas.py             # 18 Pydantic request/response schemas
│   │   ├── routers/
│   │   │   ├── __init__.py             # Package init
...
│   │       ├── paper_trader.py         # Virtual trading engine (₹1,00,000)
│   │       ├── risk_manager.py         # 6 pre-trade safety checks + kill switch
│   │       └── broker_factory.py       # Factory function — create_broker()
│   ├── scripts/
│   │   ├── scan_hardcoded_secrets.py   # Pre-commit secret scanner (11 regex)
│   │   ├── verify_health.py            # Quick health check script
│   │   ├── test_sprint2.py             # Sprint 2 import & validation test
│   │   └── test_sprint2_live.py        # Sprint 2 live endpoint test
│   ├── .env                             # Secrets (NEVER in git)
│   ├── .env.example                     # Template with empty values (in git)
...
**Total: 26 endpoints** (2 public + 24 authenticated)
