...
export const analyzeStockTicker = async (ticker: string): Promise<AnalysisResult> => {
  try {
    ...
    // We need to map it to AnalysisResult (frontend type)
    ...
    return {
      ...
      technicals: {
        ...
        macd: res.signals.macd_signal === 'BULLISH' ? 'BULLISH' : 'BEARISH',
        ema_20: res.indicators.ema_21, // Backend uses EMA-21, mapped to frontend EMA-20
      ...
      primary_recommendation: {
        ...
        signal: res.overall_signal as any,
        ideal_entry_range: [currentPrice * 0.99, currentPrice * 1.01],
        stop_loss: res.support,
        target_prices: [res.resistance],
        risk_reward_ratio: res.support > 0 ? Math.round(((res.resistance - currentPrice) / (currentPrice - res.support)) * 10) / 10 : 2,
        confidence: res.signal_strength / 100, // Backend 0-100 -> Frontend 0-1
        reason: res.summary
      }
    };
  } catch (error: any) {
