...
import {
   ...
   Shield, Search, RefreshCw, Zap, X, TrendingUp
} from 'lucide-react';
import { getDeployedResults, unmarkDeployed, OptimizedResult, getOptimizedSymbols } from '../services/optimizedParamsStore';
...
const AutoTraderDashboard: React.FC<Props> = ({ brokerState, existingTrader }) => {
   ...
   const [deployedResults, setDeployedResults] = useState<OptimizedResult[]>([]);
   const logsEndRef = useRef<HTMLDivElement>(null);

   // Load deployed optimized results on mount
   useEffect(() => {
      setDeployedResults(getDeployedResults());
   }, []);
   ...
   // Load optimized symbols into bot watchlist
   const handleLoadOptimized = () => {
      const deployed = getDeployedResults();
      if (deployed.length === 0) return;
      const symbols = [...new Set(deployed.map(r => r.symbol))];
      setConfig(prev => ({ ...prev, symbols }));
      if (existingTrader && !isRunning) {
         existingTrader.updateConfig({ ...config, symbols });
      }
   };

   // Remove a deployed result
   const handleRemoveDeployed = (symbol: string, strategy: string) => {
      unmarkDeployed(symbol, strategy);
      setDeployedResults(getDeployedResults());
   };
         ...
         <div className="space-y-6">
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
               ...
               <button
                  ...
                  className={`w-full mb-4 py-2.5 rounded-lg font-bold flex items-center justify-center gap-2 transition-all shadow-lg text-sm ${isScanning
               ...
               <div className="space-y-3">
                  <div>
                     ...
                     <div className="w-full bg-slate-800 border border-slate-700 rounded p-2 font-mono text-xs flex flex-wrap gap-1.5 min-h-[36px]">
                        {config.symbols.map(sym => {
                           const isOptimized = getOptimizedSymbols().includes(sym);
                           return (
                              <span key={sym} className={`px-2 py-0.5 rounded text-xs font-bold ${isOptimized
                                    ? 'bg-amber-500/20 text-amber-300 border border-amber-500/30'
                                    : 'bg-slate-700 text-slate-300'
                                 }`}>
                                 {isOptimized && <Zap className="w-2.5 h-2.5 inline mr-0.5" />}
                                 {sym}
                              </span>
                           );
                        })}
            ...
            {deployedResults.length > 0 && (
               <div className="bg-gradient-to-br from-amber-500/5 via-slate-900 to-slate-900 border border-amber-500/20 rounded-xl p-5">
                  <div className="flex items-center justify-between mb-3">
                     <h3 className="text-white font-bold flex items-center gap-2 text-sm">
                        <Zap className="w-4 h-4 text-amber-400" /> Optimized Strategies
                     </h3>
                     <button
                        onClick={handleLoadOptimized}
                        disabled={isRunning}
                        className="bg-amber-600 hover:bg-amber-500 text-white px-3 py-1.5 rounded-lg text-[11px] font-bold flex items-center gap-1.5 transition-all disabled:opacity-50"
                     >
                        <Play className="w-3 h-3" /> Load to Watchlist
                     </button>
                  </div>
                  <div className="space-y-2">
                     {deployedResults.map((r, i) => (
                        <div key={`${r.symbol}-${r.strategy}-${i}`} className="bg-slate-800/60 border border-slate-700/50 rounded-lg px-3 py-2.5 flex items-center justify-between gap-2">
                           <div className="flex items-center gap-3 min-w-0">
                              <div className="flex-shrink-0">
                                 <div className="text-white font-bold text-sm">{r.symbol}</div>
                                 <div className="text-slate-500 text-[10px]">{r.strategyDisplay}</div>
                              </div>
                           </div>
                           <div className="flex items-center gap-2 flex-shrink-0">
                              <span className={`px-2 py-0.5 rounded text-xs font-bold font-mono ${r.returnPct >= 0
                              ...
                              <span className="text-slate-600 text-[10px]">
                                 {new Date(r.optimizedAt).toLocaleDateString()}
                              </span>
                              <button
                                 onClick={() => handleRemoveDeployed(r.symbol, r.strategy)}
                                 className="p-1 hover:bg-slate-700 rounded transition-colors text-slate-500 hover:text-rose-400"
                                 title="Remove from Auto-Bot"
                              >
                                 <X className="w-3 h-3" />
                              </button>
                           </div>
         ...
         <div className="lg:col-span-2 space-y-6">
            <div className="bg-[#0b1120] rounded-xl border border-slate-800 overflow-hidden flex flex-col h-[300px]">
               ...
               <div className="flex-1 overflow-y-auto p-4 font-mono text-xs space-y-1">
                  {logs.map((log, i) => (
                     <div key={i} className={`border-b border-slate-800/30 pb-1 mb-1 ${log.includes('ERROR') || log.includes('❌') ? 'text-rose-400' :
