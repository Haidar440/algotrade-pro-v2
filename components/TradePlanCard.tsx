...
import { PrimaryRecommendation, TargetLevel, VolumeAnalysis } from '../types';
import { Crosshair, ShieldAlert, Target, PlayCircle, MousePointer2, Clock, BarChart3, Info } from 'lucide-react';
...
interface TradePlanCardProps {
  ...
  // Backend-driven optional fields
  tradeType?: string;
...
const TradePlanCard: React.FC<TradePlanCardProps> = ({
  ...
  const levels = useMemo(() => {
    ...
    // Use backend S/R-based targets when available
    if (backendTargets && backendTargets.length > 0) {
      return {
        stopLoss: sl,
        targets: backendTargets.map(t => t.price),
        targetLogics: backendTargets.map(t => t.logic || ''),
      };
    }

    // Fallback: compute synthetically
    const tp1 = (plan.target_prices && plan.target_prices.length > 0) ? plan.target_prices[0] : currentPrice * 1.05;
    const risk = Math.abs(currentPrice - sl);
    const tp2 = plan.signal === 'BUY' ? tp1 + risk : tp1 - risk;
    const tp3 = plan.signal === 'BUY' ? tp2 + risk : tp2 - risk;

    return {
      stopLoss: sl,
      targets: [tp1, tp2, tp3],
      targetLogics: [] as string[],
    };
  }, [plan, currentPrice, backendTargets]);

  const { stopLoss, targets, targetLogics } = levels;
    ...
    <div className="glass-panel rounded-xl p-6 border border-slate-700 bg-slate-900/50 shadow-xl">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4 border-b border-slate-700/50 pb-4">
        <div>
          ...
          <div className="flex items-center gap-2 ml-7 mt-1">
            <span className="text-xs text-slate-500">R:R <span className="text-slate-300 font-mono">1:{plan.risk_reward_ratio || '2.0'}</span></span>
            {tradeType && (
              <span className={`text-[9px] px-1.5 py-0.5 rounded font-bold uppercase ${tradeType === 'INTRADAY' ? 'bg-amber-500/20 text-amber-400 border border-amber-500/30' :
                tradeType === 'BTST' ? 'bg-blue-500/20 text-blue-400 border border-blue-500/30' :
                  tradeType === 'SWING' ? 'bg-purple-500/20 text-purple-400 border border-purple-500/30' :
                    'bg-cyan-500/20 text-cyan-400 border border-cyan-500/30'
                }`}>
                {tradeType}
              </span>
            )}
            {expectedHolding && <span className="text-[9px] text-slate-500 flex items-center gap-0.5"><Clock className="w-2.5 h-2.5" />{expectedHolding}</span>}
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="space-y-4">
          ...
          {/* Row 1: Entry Zone */}
          <div className={`p-4 rounded-xl border transition-all ${currentPrice >= entryLow && currentPrice <= entryHigh
            ...
            {entryLogic && <p className="text-[10px] text-slate-500 mt-1 flex items-center gap-1"><Info className="w-3 h-3 text-slate-600" />{entryLogic}</p>}
          </div>
          ...
          <div className="p-4 rounded-xl bg-rose-500/10 border border-rose-500/20">
            <div className="flex justify-between items-start mb-1">
              ...
              <div className="text-[10px] text-rose-400 font-bold">
                -{(riskPercent || ((Math.abs(currentPrice - stopLoss) / currentPrice) * 100)).toFixed(2)}% Risk
              </div>
            </div>
            <div className="text-xl font-mono text-rose-100 font-bold">{stopLoss.toFixed(2)}</div>
            {stopLossReason && <p className="text-[9px] text-rose-300/50 mt-1">{stopLossReason}</p>}
          </div>
            ...
            {targets.map((target, idx) => (
              <div key={idx} className="flex justify-between items-center p-3 rounded-lg bg-slate-800/30 border border-slate-700/50 hover:bg-slate-800/50 transition-colors">
                <div>
                  ...
                  {targetLogics[idx] && <span className="text-[9px] text-slate-600 ml-6 block">{targetLogics[idx]}</span>}
                </div>
          ...
          {/* Volume Confirmation */}
          {volume && (
            <div className={`p-3 rounded-lg border text-xs flex items-center gap-2 ${volume.is_volume_confirming ? 'bg-emerald-500/5 border-emerald-500/20 text-emerald-400' : 'bg-amber-500/5 border-amber-500/20 text-amber-400'
              }`}>
              <BarChart3 className="w-4 h-4" />
              <div>
                <span className="font-bold">Vol: {volume.volume_ratio?.toFixed(1)}x avg</span>
                <span className="text-slate-500 ml-2">{volume.volume_trend}</span>
                {volume.is_volume_confirming && <span className="ml-2 text-emerald-500 font-bold">✓ Confirmed</span>}
              </div>
            </div>
          )}
        </div>
