...
const StockDetailView: React.FC<StockDetailViewProps> = ({
   ...
   return (
      <div className="space-y-6 pb-20 animate-in fade-in duration-500">
         ...
         <div>
            {(() => {
               const active = result.strategies_evaluated.filter(s => s.signal === 'BUY' || s.signal === 'SELL');
               const inactive = result.strategies_evaluated.filter(s => s.signal !== 'BUY' && s.signal !== 'SELL');
               const best = active.length > 0 ? active.sort((a, b) => b.quality_score - a.quality_score)[0] : null;
               const bias = active.filter(s => s.signal === 'BUY').length > active.filter(s => s.signal === 'SELL').length
                  ? 'BULLISH' : active.filter(s => s.signal === 'SELL').length > 0 ? 'BEARISH' : 'NEUTRAL';
               ...
               return (
                  <>
                     ...
                     <div className="flex items-center justify-between mb-4">
                        <h3 className="text-xl font-bold text-white">Strategy Matrix</h3>
                        <div className="flex items-center gap-3">
                           {result.strategies_evaluated.length > 0 && (
                              <>
                                 <span className="text-xs bg-slate-800 px-2 py-1 rounded-lg text-slate-400">
                                    <span className={active.length > 0 ? 'text-emerald-400 font-bold' : 'text-slate-500'}>{active.length}</span>/{result.strategies_evaluated.length} Active
                                 </span>
                                 {best && (
                                    <span className="text-xs bg-emerald-500/10 text-emerald-400 px-2 py-1 rounded-lg font-medium">
                                       Best: {best.strategy_name} ({Math.round(best.quality_score * 100)}%)
                                    </span>
                                 )}
                     ...
                     {result.strategies_evaluated.length === 0 ? (
                        <div className="p-6 rounded-xl border border-slate-700/50 bg-slate-800/30 text-center">
                           ...
                           <p className="text-sm text-slate-500">{result.primary_recommendation.reason || 'Insufficient data for strategy analysis.'}</p>
                        </div>
                     ) : (
                        <>
                           ...
                           {active.length > 0 && (
                              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 mb-4">
                                 {active.sort((a, b) => b.quality_score - a.quality_score).map((strat, idx) => (
                                    <StrategyCard key={`active-${idx}`} strategy={strat} />
                                 ))}
                              </div>
                           )}
                           ...
                           {inactive.length > 0 && (
                              <>
                                 ...
                                 >
                                    {showInactive ? '▾ Hide' : '▸ Show'} {inactive.length} inactive strategies
                                 </button>
                                 {showInactive && (
                                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 opacity-60">
                                       {inactive.map((strat, idx) => (
                                          <StrategyCard key={`inactive-${idx}`} strategy={strat} />
                                       ))}
                                    </div>
                                 )}
                              </>
