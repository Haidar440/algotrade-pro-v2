    ...
    <div className="md:hidden fixed bottom-0 left-0 right-0 bg-[#0f172a] border-t border-slate-800 z-50 pb-safe">
      <div className="flex justify-around items-center h-16">
        <button
          ...
          className={`flex flex-col items-center justify-center w-full h-full space-y-1 ${activeView === 'DASHBOARD' ? 'text-emerald-400' : 'text-slate-500'}`}
        >
        ...
        <button
          onClick={() => onNavigate('REAL_PORTFOLIO')}
          className={`flex flex-col items-center justify-center w-full h-full space-y-1 ${activeView === 'REAL_PORTFOLIO' ? 'text-emerald-400' : 'text-slate-500'}`}
        >
